...
in
{
  perSystem =
    ...
    }:
    {
      ...
      legacyPackages = lib.genAttrs config.ghc.names.all (ghcVersion: {
        all-checks = pkgs.symlinkJoin {
          name = "all-checks-${ghcVersion}";
          paths =
            map (x: x.package) (builtins.attrValues config.haskellProjects.${ghcVersion}.outputs.packages)
            ++ builtins.attrValues config.haskellProjects.${ghcVersion}.outputs.checks;
        };
      });
    };
}
